...
- **Frontend strategy (GOVERNANCE — read before any UI work):** `docs/architecture/frontend-strategy.md` — official sourcing & adaptation law (RetailOS **UI Platform**, the **7-layer stack**: AdminCN shell → **CommerceO** commerce workflows → shadcn **Studio Blocks** → shadcn **Studio Components** (default primitive library) → shadcn/ui fallback primitives → Magic UI motion → RetailOS custom; every block becomes owned code in `packages/ui`, wired to oRPC, re-themed to RetailOS tokens, stripped of mock/Next/auth/routing assumptions; AdminCN + CommerceO are the visual targets, the RetailOS backend stays authoritative). The **per-module source registry** + Screen Composition Matrix is `docs/architecture/ui-source-registry.md`; the **per-component preferred source** is `docs/architecture/component-preference-matrix.md`; **vertical onboarding presets** (platform-not-product) are `docs/architecture/vertical-presets.md`; the design *law* is the `retailos-design-language` skill; AdminCN/CommerceO ZIP detail is `ui-admin-shell-findings.md` + `frontend-strategy.md` §1. **No production UI until APIs are stable + approved.**
