...
const NAV = [
  ...
  { label: "Television", to: "/shop", search: { category: "television" } },
  {
    ...
    to: "/shop",
    search: { category: "air-conditioning" },
  },
];
  ...
  return (
    <header className="sticky top-0 z-40 border-border/70 border-b bg-background/85 backdrop-blur-md">
      {IS_MOCK_DATA ? (
        <div className="flex items-center justify-center gap-2 bg-primary/10 px-4 py-1.5 text-center text-[11px] text-primary">
          ...
          <span className="text-primary/70">
            Showing sample catalog data — wires to the live commerce API with
            one flag.
          </span>
        </div>
      ) : null}
      ...
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center gap-4 px-4 sm:px-6">
        ...
        {/* biome-ignore lint/a11y/useSemanticElements: role="search" on the form retains broader assistive-tech support than the newer <search> landmark */}
        <form
        ...
        >
          ...
          {count > 0 ? (
            <Badge className="absolute -top-1.5 -right-1.5 flex size-5 items-center justify-center rounded-full p-0 text-[10px] tabular-nums">
              {count}
            </Badge>
          ) : null}
        </Button>
