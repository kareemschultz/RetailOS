...
- **Current step:** **Phase 2 ✅ COMPLETE + merged to master @ `72b2100` (PR #9), CI green (4/4 incl. real-Postgres RLS).** Full Phase-2 commit-by-commit narrative archived in [`phase-2-complete.md`](./phase-2-complete.md). **PARKED → later phases:** #8 POS↔costing wiring + `cost_reconciliation` emit (Phase 4); #6 precision/`mulDivRound` + `valuation_updated` totalValue/qtyOnHand enrichment (Phase 5); #7 set-once DB-trigger backstop + #5 composite-FK `(tenant_id,id)` (Phase 3). **Next: Phase 3 (Locations / Warehouses / Bonds) — NOT started; kickoff is a separate owner GO.**
