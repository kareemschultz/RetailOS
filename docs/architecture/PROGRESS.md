...
- **Current step:** **Phase 3 (Locations / Warehouses / Bonds) — PLANNING PACKET DRAFTED; awaiting owner approval before any schema/code.** Phase 2 ✅ complete + merged @ `72b2100`, CI green; narrative archived in [`phase-2-complete.md`](./phase-2-complete.md). Phase-3 docs: `phase-3-implementation-plan.md`, `phase-3-gap-analysis.md`, `module-specs/locations-warehouses-bonds.md`, `event-map-phase3.md`, `competitive/locations-warehouses-bonds.md`. **#5 composite-FK is IN SCOPE for Phase 3** (new tables born with `FK (tenant_id, x_id)`; referenced tables get `UNIQUE(tenant_id, id)`). **Load-bearing design question = transfer VALUE conservation** (move the exact integer released value `V`; additive touch to frozen `costing.ts`). **6 open decisions need owner answers** (in-transit modelling, bond-release mechanics, bins scope, FIFO transfer cost basis, frozen-costing additive approval, transfer oversell — plan §I). Recommended first commit = composite-FK groundwork (`UNIQUE(tenant_id,id)`, expand-only). **STILL PARKED:** #8 + `cost_reconciliation` (Phase 4); #6 + `valuation_updated` enrichment (Phase 5); #7 set-once DB-trigger (Phase 3 backstop).
