...
- **Current step:** **Phase 3 — COMMIT 0 IMPLEMENTED (parked debt #5 + #7); STOPPED for owner review before commit 1.** Plan merged to master (PR #11, `211edaa`); commit 0 on branch `phase-3-commit-0` (off master). All 6 design decisions LOCKED (plan §I; §I.2/§I.3 are the owner-confirmed F1/F2 versions). **Commit 0 (migration 0012, expand-only):** `UNIQUE(tenant_id,id)` on company/location/product/sku/lot + `UNIQUE(tenant_id,company_id,id)` on location (F3) + an **additive** composite FK `location(tenant_id,company_id)→company(tenant_id,id)` (existing plain FK kept) — proven to make a cross-tenant `location→company` ref **DB-layer-impossible** (raw-insert test); + the **#7 set-once costing-method DB-trigger** on product/sku with **F4** nullable-`sku_id` coverage (a product-level movement locks the SKU too) — proven by a raw-UPDATE-after-movement test. **Gates:** check 158/0 · check-types 6/6 · test 7/7 · mojibake clean · DB-gated migrate 0000–0012 + **db 45/45 (regression, unchanged) + api 13/13 (+2 commit-0 tests)** · coverage gate green (no new tables). Lesson recorded: drizzle-kit emits a composite FK before its target UNIQUE → hand-reorder. **Next: commit 1 (location self-ref tree + flags + capacity seam) — awaits owner GO.** **STILL PARKED elsewhere:** #8 + `cost_reconciliation` (Phase 4); #6 + `valuation_updated` enrichment (Phase 5).
