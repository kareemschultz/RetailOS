...
- **Current step:** **Phase 2 implementation in progress.** ✅ **Commit 0 landed** (mechanical RLS coverage gate). ✅ **Commit 1 landed** — catalog schema + migration + RLS. ✅ **Commit 2 landed** — tracking schema + stock ledger lot/serial/cost seams. ✅ **Commit 3 landed** — costing storage (`avg_cost`, `valuation_layer`) + tenant config columns. ✅ **Commit 4 landed** — reorder rules, stock counts, bundle/BOM schema + migration + RLS. ✅ **Commit 5 landed** — raw rich Phase-2 seed foundation. ✅ **Commit 6 landed** — SKU ledger seam + costing resolver/applyValuation service with DB-gated AVCO/FIFO tests. ✅ **Commit 7 ready/landed in this branch** — valuation-bearing seed movements through resolver. **Next: UoM/FEFO/oversell/reorder/count services.**

### ⛔ BLOCKERS / gates

**Phase 2 implementation is APPROVED and underway in commit order.** All product-policy decisions D1–D7 are LOCKED (below); only D-money rounding stays open (Phase 5, not a Phase-2 blocker — AVCO carries exact-integer remainders, FIFO is division-free; see the plan's *Value-integrity invariants*). Phase-2 schema groups, raw rich seed data, SKU-level ledger seam, and costing resolver/applyValuation service now exist; **no UoM/FEFO/oversell/reorder/count services, routers, or UI yet.**
