...
| INV-2 | **Transfer VALUE conservation** — value released at source == value received at destination; total value unchanged. Isolated **per-transfer** (Codex F2): each transfer's own in-transit node prevents value-blending across concurrent same-SKU transfers. FIFO uses a value-exact layer set (Codex F1), not a lossy single layer. | `transfer.ts` calling `applyValuation` on BOTH legs + the exact-`V` mechanism (plan §C) | `transfer` router → valuation both legs | **#8-class grep** + value-conservation DB test (`Δvalue_A = −Δvalue_B = V`) for AVCO **and** FIFO, incl. two overlapping same-SKU transfers. |
...
| INV-5 | **Duty cost-basis add (intentional value-add, NOT conservation)** — on release, duty/tax is added to the released cost basis via the value-only `valuation_adjustment` seam (qty unchanged). Durable AVCO guarantee (Codex F5): the resolved costing method is **stamped on `bond_receipt_line` at receipt** and release enforces against the stamp (immune to later category/tenant costing changes). | `bond.ts` invoking the existing value-only path | release router | DB test: released `total_value_minor` += exactly `duty+tax`; qty unchanged; qty=0⟺value=0 preserved; a post-receipt tenant→FIFO flip does NOT break release. |
| INV-6 | **Intra-company only** — a transfer's source & dest share `company_id`; inter-company blocked. **DB-enforced** (Codex F3): `company_id` denormalized on `stock_transfer` + composite FK `(tenant_id, company_id, location_id) → location(tenant_id, company_id, id)` on both endpoints (a CHECK can't read the referenced row). | DB composite FK + `transfer.ts` (belt-and-braces) | `transfer` router | DB test: cross-company transfer rejected at the DB layer (raw insert too). |
...
| INV-8 | **Set-once costing method (DB backstop #7)** — `costing_method` immutable once ledger rows exist, at the DB layer. Trigger covers nullable `stock_ledger.sku_id` (Codex F4): SKU update blocks on `sku_id = NEW.id` OR a product-level row (`sku_id IS NULL`) for `NEW.product_id`; product update checks all rows by `product_id`. | DB trigger + app `assertCostingMethodSetOnce` | all costing-method writes | DB test: raw UPDATE rejected after a movement — both SKU-direct and product-level-only cases. |
