...
4. **FIFO value-only allocation:** rejected for now (OPEN). **⚠️ When built, the FIFO value-only path MUST carry its OWN `qty_on_hand > 0` guard** — the Gap-A guard lives in `applyAvcoValueOnly` and reads `avg_cost`, so it does **NOT** cover FIFO (which reads `valuation_layer`). The zero-stock invariant is currently enforced by **two different mechanisms** — the AVCO guard *and* the FIFO outright-reject — so a future FIFO value-only change could silently reopen the zero-stock hole if it forgets the guard. (Tracked on the FIFO-value-only ticket.)
5. **D-money rounding mode** (unchanged) — value-only adjustments + cost-reconciliation true-ups grow `total_value_minor` without qty, **accelerating the issue-#6 2^53 precision exposure**. **`cost_reconciliation` true-ups are the LARGEST single value-injection** (on the zero/unvalued oversell basis the reconciliation amount ≈ full receipt cost of the previously-unvalued units), so the rounding-mode / `mulDivRound` decision (#6) is a **HARD DEPENDENCY of the `cost_reconciliation` emit**, not a parallel nicety.
6. **Set-once durable guard:** the set-once rule is enforced at the router layer only (application-level). A DB-level trigger (reject `costing_method` UPDATE when `stock_ledger` rows exist) is the durable class-level fix so a future service path can't bypass it — tracked as a follow-up ticket (mirrors the H1→#5 composite-FK durable fix).
