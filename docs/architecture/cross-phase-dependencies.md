...
>
> **Commission posting dimension + POST-1/POST-2 invariants (gate 4, 2026-06-24).** Gate 4 found the GL's **commission accrual** journal (Dr commission-expense / Cr commission-payable) had **no field** on any P4 event — a missing posting *dimension*, not a missing field. Root cause: the event map was being built by enumerating remembered postings. Fixed structurally by governing the contract with two checkable invariants (in `event-map-phase4.md`): **POST-1** (every monetary quantity is the full quintuple — `*Minor`+`currency`+`scale`+`*FunctionalMinor`+FX-rate-used) and **POST-2** (the contract enumerates EVERY journal the GL posts per event, and every enumerated amount travels on the event). Commission rides **Decision #7** `commission_accrual_policy`: `at_sale` → fields on `sale.created` (clawback on `sale.refunded` proportional / `sale.voided` 100% via original-journal reversal); `at_settlement` → fields on `payment.received` (reverse on cancellation). Like the FX rate, commission is computed from **MUTABLE rules** so it is **stamped at the accrual trigger** (reserved-nullable: `commission*`, `lineCommission*`, `commissionClawback*`, `commissionAccrualPolicy`); refunds derive clawback from the original line's stamp, never re-resolve. The **commission_payable** account + the accrual-policy setting are the P5/Decision-#7 consumers.
