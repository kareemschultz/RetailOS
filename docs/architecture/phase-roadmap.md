...
| 4 — POS & Offline Queue | sale mutation, payments, receipts, shifts, offline queue, idempotency, number blocks, Tauri POS | **📋 PLANNING COMPLETE / implementation-ready (NOT built)** — research done, Codex-reviewed (3 CRIT + 4 HIGH folded), **all 7 🔒 decisions LOCKED** (`phase-4-implementation-plan.md` §12), `event-map-phase4.md` locked for the P5 GL consumer, readiness verdict ✅ (§15). Build = a separate session. Docs: plan, `event-map-phase4.md`, `competitive/pos.md`, `cross-phase-dependencies.md`. |
