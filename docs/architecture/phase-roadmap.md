...
| 2 — Products & Inventory Ledger | products/variants/SKU/barcode, stock ledger, multi-UoM, serial/batch/expiry | **✅ Backend review-ready for approved scope** — §41/§42 docs + ADR-0007 merged; D1–D7 locked; schema/RLS/seed/services/routers/API contracts/tests complete for approved scope; catalog/product/variant/lot/reorder lifecycle routes + discrepancy/revaluation/import-preview seams implemented; **no P0 backend gaps remain**. See `phase-2-gap-analysis.md` and `phase-2-closeout-audit.md`. Production UI and downstream phase consumers intentionally absent. |
