...
2. **Receive at destination at exactly `V`:**
   ...
   - **FIFO — needs an ADDITIVE costing primitive (flagged):** the destination must create a layer carrying exactly `V`. The existing FIFO receipt creates one layer at `unitCost·q` (rounds) and FIFO value-only is *rejected* today. Cleanest = an **additive** `transfer_in` branch that creates the destination layer(s) from the source's consumed-layer cost(s) (layer-preserving, exact). **This is the only place a plain transfer touches frozen `costing.ts`.**

**Frozen-costing impact — flagged explicitly:**
- **AVCO transfers: zero frozen change** (reuse receipt + value-only seams).
- **FIFO transfers: one ADDITIVE primitive** in `costing.ts` (new branch; existing receipt/issue/adjust/value-only behaviour unchanged). **Owner decision:** accept as a **Phase-3 additive** (recommended — additive, value-conservation-tested, earns a line-by-line read) vs a **Phase-2 change-request** (its own mini-pass).
...
- **DUTY/TAX on release reuses the value-only `valuation_adjustment` seam (no new costing machinery):** after the release transfer lands the stock at the released location, a **value-only `valuation_adjustment`** (`qty_delta=0`, `value_delta = duty + taxes`) **adds the duty/tax to the cost basis** at the released location. This is the existing AVCO value-only path — duty is a real added cost, intentionally *not* conserved (distinct from the transfer in §C). 
  - **FIFO caveat (flagged):** value-only is AVCO-only today (FIFO value-only is the parked OPEN decision). So duty-on-release works for AVCO SKUs via the existing seam; **bonded FIFO SKUs** need the FIFO value-only path — restrict bonded stock to AVCO in Phase 3, or implement FIFO value-only (a costing touch). **Owner decision.**
...
1. **Stock grain:** post stock at stock-holding-node grain (recommended; zero frozen-costing change) vs bin-grained stock now (frozen-touching).
2. **FIFO transfer cost basis / frozen touch:** accept the additive FIFO `transfer_in` primitive as a Phase-3 additive (recommended) vs a Phase-2 change-request; layer-preserving vs aggregate-layer.
3. **In-transit modelling:** virtual `is_transit` location (recommended) — confirm value flows through it vs value escrowed on the transfer line.
4. **Bond-release mechanics:** RBAC-gated immediate vs request→approve workflow; and **bonded FIFO** handling (restrict bonded to AVCO in P3 vs implement FIFO value-only).
5. **Bins:** structure + capacity seam only in Phase 3 (recommended) vs bin-level stock balances now.
6. **Transfer oversell:** hard-block at source regardless of D5 (recommended — can't ship stock you don't have) vs honour D5 per location.
