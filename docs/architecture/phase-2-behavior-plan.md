...
2. **`inventory.cost_reconciliation` emit (negative→non-negative).** Emit on the receipt path when a SKU×location crosses from negative on-hand to ≥0 and the receipt establishes actual cost.
   - **⛔ UPSTREAM BLOCKER — POS↔costing boundary (ticket #8) must be resolved FIRST.** Reconciliation true-ups COGS on the valuation projection, but the primary sale path (`pos.createSale`) does **not** update that projection today (no `applyValuation`; `avg_cost.qty_on_hand` diverges from the ledger, no FIFO consumption, no COGS). Reconciling on a projection that POS sales don't maintain is building on a **known-wrong base** — the negative-on-hand state itself isn't valued consistently. Resolve #8 (wire the sale/issue path through the engine, or formally scope it Phase 4) before this item.
